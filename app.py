...
def precipitation():
    """Query to retrieve only the last 12 months of precipitation data."""
    # Create our session (link) from Python to the DB.
    session = Session(engine)

  
    last_measurement_data_tuple = session.query(
        Measurement.date).order_by(Measurement.date.desc()).first()
    (latest_date, ) = last_measurement_data_tuple
    latest_date = dt.datetime.strptime(latest_date, '%Y-%m-%d')
    latest_date = latest_date.date()
    date_year_ago = latest_date - relativedelta(years=1)

  
    data_last_year = session.query(Measurement.date, Measurement.prcp).filter(
